...
export const useFlowStore = defineStore('flow-store', () => {
   ...
   * 初始化流程图数据
   * 创建一个默认的开始节点作为流程图的起点
   * @param {string} templateId - 可选的模板ID，指定加载哪个模板
   */
  const initFlowData = (templateId?: string) => {
    let templateData = null
    if (templateId) {
      templateData = getTemplateFlow(templateId)
    }
    if (!templateData) {
      templateData = JSON.parse(JSON.stringify(MockData))
    }
    templateData.name =
      "工作流（" + formatDate(new Date(), "yyyy/MM/dd HH:mm:ss") + "）";
    flowData.value = templateData;
  };
   ...
   */
  const initFlowDataByTemplate = (templateId: string) => {
    initFlowData(templateId)
  };
  ...
  return {
    ...
    initFlowDataByTemplate, // 根据模板ID初始化流程图数据
